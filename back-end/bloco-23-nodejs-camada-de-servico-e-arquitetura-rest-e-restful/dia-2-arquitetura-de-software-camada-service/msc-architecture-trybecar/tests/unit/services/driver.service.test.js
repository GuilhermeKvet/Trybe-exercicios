...
const validDriverName = 'Gui';
const invalidValue = '1';

describe('Verificando service motorista', function () {
  describe('Lista os motoristas cadastrados', function () {
      it('É retornado a resposta com os motoristas', async function () {
        sinon.stub(driverModel, 'findAll').resolves(driverList);

        const driver = await driverService.getDrivers();

        expect(driver).to.deep.equal(driver);
      });
  });
  afterEach(function () {
    sinon.restore();
  ...
  describe('Verificando service Driver', function () {
    ...
    describe('Cadastrando uma nova pessoa motorista com sucesso sem carros', function () {
      beforeEach(function () {
        sinon.stub(driverModel, 'registerNewDriver').resolves(createdDriver);
      });
    ...
    describe('Cadastrando uma nova pessoa motorista com sucesso com carros', function () {
      beforeEach(function () {
        sinon.stub(driverModel, 'registerNewDriver').resolves(createdDriver);
        sinon.stub(driveCarModel, 'insert').resolves();
