...
class TransferController {
  ...
  public async findAllTransfers() {
    try {
      const allTransfers = await this.service.findAllTransfers();
  ...
  public async findTransferByKey() {
    try {
      const userTransfers = await this.service.findTransfersByKey(this.req.params);
      return this.res.status(200).json(userTransfers);
    } catch (error) {
      this.next(error);
    }
  }
